-- =====================================================
-- DELIVERABLES SYSTEM TABLES (Fixed Version)
...
CREATE POLICY "Authenticated users can insert deliverables" ON deliverables
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated users can update deliverables" ON deliverables
  FOR UPDATE USING (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated users can delete deliverables" ON deliverables
  FOR DELETE USING (auth.uid() IS NOT NULL);

-- Deliverable versions
CREATE POLICY "Authenticated users can view versions" ON deliverable_versions
  FOR SELECT USING (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated users can manage versions" ON deliverable_versions
...
-- Approval records
CREATE POLICY "Authenticated users can view approvals" ON approval_records
  FOR SELECT USING (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated users can insert approvals" ON approval_records
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

-- Activity feed
CREATE POLICY "Authenticated users can view activity" ON activity_feed
  FOR SELECT USING (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated users can insert activity" ON activity_feed
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

-- Health reports
CREATE POLICY "Authenticated users can view health reports" ON project_health_reports
  FOR SELECT USING (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated users can manage health reports" ON project_health_reports
  FOR ALL USING (auth.uid() IS NOT NULL);

-- Monitoring settings
CREATE POLICY "Authenticated users can view monitoring settings" ON project_monitoring_settings
  FOR SELECT USING (auth.uid() IS NOT NULL);

CREATE POLICY "Authenticated users can manage monitoring settings" ON project_monitoring_settings
  FOR ALL USING (auth.uid() IS NOT NULL);
