...
export function Sidebar() {
  ...
  const [isClientsSubExpanded, setIsClientsSubExpanded] = useState(false)
  const [isSigningOut, setIsSigningOut] = useState(false)
  const { activeClients, loading: clientsLoading } = useClientNavigation()
  const { signOut } = useAuth()

  // Auto-expand projects section if on a project or client route
  useEffect(() => {
    if (pathname.startsWith('/projects') || pathname.startsWith('/clients')) {
      setIsProjectsExpanded(true)
    }
    if (pathname.startsWith('/clients')) {
      setIsClientsSubExpanded(true)
    }
  }, [pathname])
      ...
      <div className="relative flex flex-col h-full">
        ...
        <nav className="flex-1 py-6 px-3 space-y-1 overflow-y-auto">
          {navigation.map((item, index) => {
            const isActive = pathname === item.href || (item.href !== '/dashboard' && pathname.startsWith(item.href)) || (item.expandable && pathname.startsWith('/clients'))
                      ...
                      {isProjectsExpanded && !isCollapsed && (
                        ...
                        >
                          <div className="pl-6 pt-2 pb-1 space-y-1">
                            ...
                            <div>
                              <button
                                onClick={() => setIsClientsSubExpanded(!isClientsSubExpanded)}
                                className={cn(
                              ...
                              >
                                ...
                                <motion.div
                                  animate={{ rotate: isClientsSubExpanded ? 180 : 0 }}
                                  transition={{ duration: 0.2 }}
                                >
